        ...
        for opp in opportunities:
            try:
                ...
                if not is_valid:
                    ...
                    continue
                content, rationale, _source_prompt, metadata = copilot.generate_reply(opp, brand_dict, prompts)
                ...
                opp_status = "drafting"
                if metadata.get("risk_flags") or metadata.get("confidence_score", 0.0) < 0.7:
                    opp_status = "drafting"  # Keeping it in drafting but could be flagged in UI
                
                update_opportunity(db, opp["id"], {"status": opp_status})
                drafts_count += 1
            except Exception as e:
