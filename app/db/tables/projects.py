...
def create_project(db: Client, project_data: dict[str, Any]) -> dict[str, Any]:
    ...
    result = db.table(PROJECTS_TABLE).insert(data).execute()
...
def update_project(db: Client, project_id: int, update_data: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if "status" in data:
        data["is_active"] = (data.pop("status") == "active")
    result = db.table(PROJECTS_TABLE).update(data).eq("id", project_id).execute()
    return result.data[0] if result.data else None
...
def get_prompt_template_by_id(db: Client, template_id: int) -> dict[str, Any] | None:
    ...
    data = result.data[0]
    if "type" in data:
        data["prompt_type"] = data.pop("type")
    if "user_prompt_template" in data:
        data["instructions"] = data.pop("user_prompt_template")
    return data
...
def get_default_prompts(db: Client, project_id: int) -> list[dict[str, Any]]:
    ...
    prompts = list(result.data)
    for p in prompts:
        if "type" in p:
            p["prompt_type"] = p.pop("type")
        if "user_prompt_template" in p:
            p["instructions"] = p.pop("user_prompt_template")
    return prompts


def create_prompt_template(db: Client, template_data: dict[str, Any]) -> dict[str, Any]:
    """Create a new prompt template."""
    db_data = dict(template_data)
    if "prompt_type" in db_data:
        db_data["type"] = db_data.pop("prompt_type")
    if "instructions" in db_data:
        db_data["user_prompt_template"] = db_data.pop("instructions")
    result = db.table(PROMPT_TEMPLATES_TABLE).insert(db_data).execute()
    data = result.data[0]
    if "type" in data:
        data["prompt_type"] = data.pop("type")
    if "user_prompt_template" in data:
...
def update_prompt_template(db: Client, template_id: int, update_data: dict[str, Any]) -> dict[str, Any] | None:
    ...
    db_data = dict(update_data)
    if "prompt_type" in db_data:
        db_data["type"] = db_data.pop("prompt_type")
    if "instructions" in db_data:
        db_data["user_prompt_template"] = db_data.pop("instructions")
    result = db.table(PROMPT_TEMPLATES_TABLE).update(db_data).eq("id", template_id).execute()
    if not result.data:
        return None
    data = result.data[0]
    if "type" in data:
