...
- **Embeddings caveat**: TF-IDF similarity is computed pairwise per comparison (`TfidfProvider.pairwise_similarity`) — never compare vectors from different fits; the `EmbeddingService` cache only applies to corpus-independent backends (sentence-transformers).
